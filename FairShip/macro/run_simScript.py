...
theMass = 1.0*u.GeV
#https://arxiv.org/pdf/1511.07436.pdf
# figure 4. - arguements one and two are x any y axis (axis is normalized to the stop mass of 1Tev)
# 1000 is the stop mass in GeV
theCouplings = [1, 1, 1000] # ctau=53.3km  TP default for HNL
...
nEvents      = 2500
firstEvent   = 0
inclusive    = "b"    # True = all processes if "c" only ccbar -> HNL, if "b" only bbar -> HNL, if "bc" only Bc+/Bc- -> HNL, and for darkphotons: if meson = production through meson decays, pbrem = proton bremstrahlung, qcd = ffbar -> DP.
...
HNL          = False
DarkPhoton   = False
# RPV sens - set to true
RPVSUSY      = True
# this refers to the Benchmark Scenario in the paper
RPVSUSYbench = 2
leptongeneration = 1
# set default plot2mass
plot2mass = 0.6
plot1massmax = 2
# GeV units added in below lines
...
try:
        opts, args = getopt.getopt(sys.argv[1:], "D:FHPu:n:i:f:c:hqv:s:l:A:Y:i:m:co:t:g:jobid:benchmark:plot:plot2mass:plot1massmax",[\
                                   ...
                                   "production-couplings=","decay-couplings=","dry-run","jobid=","benchmark=","plot=","plot2mass=","plot1massmax=","leptongeneration=","visible=","bzerobplus="])
...
for o, a in opts:
        ...
        if o in ("-A",):
            inclusive = a
            if inclusive == 'b': #if a=='b': 
              print('Load beauty input file')
              # quit()
              inputFile = "/eos/experiment/ship/data/Beauty/Cascade-run0-19-parp16-MSTP82-1-MSEL5-5338Bpot.root"
            if inclusive == 'c': #if a=='c': 
              print('Load charm input file')
              # quit()
              inputFile = "/eos/experiment/ship/data/Charm/Cascade-parp16-MSTP82-1-MSEL4-978Bpot.root"
        if o in ("--Genie",):
            simEngine = "Genie"
        ...
        if o in ("-plot", "--plot",):
            plot = int(a)
        if o in ("-plot2mass", "--plot2mass",):
            plot2mass = float(a)
        if o in ("-plot1massmax", "--plot1massmax",):
            plot1massmax = float(a)
        if o in ("-leptongeneration", "--leptongeneration",):
            leptongeneration = int(a)
        if o in ("-visible", "--visible",):
            visible = int(a)
            if visible == 1:
              visible = True
            else:
              visible = False
        if o in ("-bzerobplus", "--bzerobplus",):
            bzerobplus = int(a)

benchmark = RPVSUSYbench


if plot == 1:
  #mass vs coupling plot
  ...
  # lambda_112_over_m2 = 10**((np.float(y_row)/50.)*1 - 9)
  ...
  # theMass = ((np.float(x_column)/50.)*(plot1massmax-2)+2)*u.GeV
  # print(theMass)
  # quit()
  print ' '
  ...
  print ' '
  print 'theCouplings',theCouplings[0]/(1000*1000), theCouplings[1]/(1000*1000)
  print 'theMass',theMass
  print ' '

  if jobid >= 2500 and jobid < 2550:
    theCouplings[0] = 1E-5*(1000*1000)
    jobid = jobid-2500
    rangea = 12 - 5
    exp = float(jobid/50.)*rangea
    theCouplings[1] = 10**(-5 - exp)*(1000*1000)
  if jobid >= 2550:
    theCouplings[1] = 1E-5*(1000*1000)
    jobid = jobid-2500+1 #+1 to avoid repeating first point
    rangea = 12 - 5
    exp = float(jobid/50.)*rangea
    theCouplings[0] = 10**(-5 - exp)*(1000*1000)

# quit()

# theMass = 3
# theCouplings = [0.1,0.1,1000]
 ...
 if HNL or RPVSUSY:
  ...
  if RPVSUSY:
   ...
   pythia8_conf.configurerpvsusy(P8gen,theMass,[theCouplings[0],theCouplings[1]],
                                theCouplings[2],RPVSUSYbench,inclusive,leptongeneration,visible,bzerobplus,deepCopy) #True/false visible
