...
class constants():
	...
	def __init__(self):
		self.decayConstant = {'K+':0.156*u.GeV,
							  ...
							  'B+':0.191*u.GeV,'B-':0.191*u.GeV, #ALEX
...
class RPVSUSYbranchings():
	...
	"""
	def __init__(self, mass, couplings, sfmass, benchmark, leptongeneration, visible,bzerobplus,debug=False):
		"""
		...
		if bzerobplus == 0:
			if leptongeneration == 2 or leptongeneration == 3:
				...
				self.decays_visible =  {1:['N -> K+ mu-','N -> K*+ mu-'],
								2:['N -> K+ mu-','N -> K*+ mu-'],
								3:['N -> K+ mu-','N -> K*+ mu-'],
								4:['N -> D+ mu-','N -> D*+ mu-'],
								5:['N -> K+ tau-','N -> K*+ tau-']}

		# is 'N -> K+ mu-' the same as 'N -> K- mu+' or are both needed?
				self.prods   =  {1:['D+ -> N mu+'],
								 2:['D_s+ -> N mu+'],
								 3:['B0 -> N nu_mu'],
								 4:['B0 -> N nu_mu'],
			...
			if leptongeneration == 1:
				self.decays =  {1:['N -> K+ e-','N -> K*+ e-','N -> K*0 nu_e','N -> K_L0 nu_e','N -> K_S0 nu_e'],
								2:['N -> K+ e-','N -> K*+ e-','N -> K*0 nu_e','N -> K_L0 nu_e','N -> K_S0 nu_e',
								   'N -> eta nu_e','N -> eta\' nu_e','N -> phi nu_e'],
								3:['N -> K+ e-','N -> K*+ e-','N -> K*0 nu_e','N -> K_L0 nu_e','N -> K_S0 nu_e'],
								4:['N -> D+ e-','N -> D*+ e-','N -> K*0 nu_e','N -> K_L0 nu_e','N -> K_S0 nu_e'],
								5:['N -> K+ tau-','N -> K*+ tau-','N -> K*0 nu_tau','N -> K_L0 nu_tau','N -> K_S0 nu_tau']}

				self.decays_visible =  {1:['N -> K+ e-','N -> K*+ e-'],
								2:['N -> K+ e-','N -> K*+ e-'],
								3:['N -> K+ e-','N -> K*+ e-'],
								4:['N -> D+ e-','N -> D*+ e-'],
								5:['N -> K+ tau-','N -> K*+ tau-','N -> K*0 nu_tau','N -> K_L0 nu_tau','N -> K_S0 nu_tau']}
	...
	def AddChannelsToPythia(self,P8Gen,verbose=True):
		...
		if self.visible == True:
			decays_tmp = self.decays_visible[self.bench]
			...
			for decay in decays_tmp:
					...
					total_visible_BR += self.findDecayBranchingRatio(decay)
			...
			for decay in decays_tmp:
					...
					print decay
					bf        = self.findDecayBranchingRatio(decay)/total_visible_BR
	...
	def Width_H_L(self, H, L):
		...
		if self.bench==1 and ('K' in H and 'nu' in L):
	...
	def Width_N_L(self, H, L):
		...
		# width=self.U2[1]*tmp_width #KOSTAS
		width=self.U2[0]*tmp_width #ALEX
	...
	def findProdBranchingRatio(self, decayString):
		...
		# br = 0.
		# totalwidth = self.NprodWidth()
		# if totalwidth > 0.0:
		#     br = self.Width_N_L(had,lep)/totalwidth


		print(had,lep)

		br = self.Width_N_L(had,lep)/(self.Width_N_L(had,lep)+c.hGeV/lifetime(had)) # MESON CAN DECAY TO final states not containing neutralinos
		# br = self.Width_N_L(had,lep)/(c.hGeV/lifetime(had) + self.Width_N_L(had,lep)) #?
...
class RPVSUSY(RPVSUSYbranchings):
	...
	def __init__(self, mass, couplings, sfmass, benchmark, leptongeneration, visible,bzerobplus, debug=False):
		...
		"""
		RPVSUSYbranchings.__init__(self, mass, couplings, sfmass, benchmark, leptongeneration, visible,bzerobplus, debug)
