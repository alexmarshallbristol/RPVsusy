...
def configurerpvsusy(P8gen, mass, couplings, sfermionmass, benchmark, inclusive, leptongeneration, visible, bzerobplus, deepCopy=False, debug=True):
  
    # configure pythia8 for Ship usage
    if debug:
        pythia_log=open('pythia8_conf.txt','w')
        P8gen = MethodLogger(P8gen, sink=pythia_log)
    if leptongeneration == 1 or leptongeneration == 3:
        h = make_interpolators(
            os.path.expandvars("$FAIRSHIP/shipgen/branchingratiosrpvsusybench{}.dat".format(benchmark)))
        print("$FAIRSHIP/shipgen/branchingratiosrpvsusybench{}.dat".format(benchmark))
    elif leptongeneration == 2:
        h = make_interpolators(
            os.path.expandvars("$FAIRSHIP/shipgen/branchingratiosrpvsusybench{}_muons.dat".format(benchmark)))
    P8gen.UseRandom3() 
    ...
    if inclusive=="c":
        ...
        print('ALEX PYTHIA8_CONF',couplings)

        P8gen.SetParameters("HardQCD::hardccbar  = on")
        # add RPVSUSY
        rpvsusy_instance = rpvsusy.RPVSUSY(mass, couplings, sfermionmass, benchmark, leptongeneration, visible, bzerobplus, debug=True)
        ...
        if leptongeneration == 1:
            ...
            if getbr_rpvsusy(h,'ds_e',mass,couplings[0])>0.:
               P8gen.SetParameters("431:addChannel      1  "+str(getbr_rpvsusy(h,'ds_e',mass,couplings[0])/maxsumBR)+\
                                   "    0      -11       9900015")
               sumBR+=float(getbr_rpvsusy(h,'ds_e',mass,couplings[0])/maxsumBR)
            ...
            sumBR=0.
            if getbr_rpvsusy(h,'d_e',mass,couplings[0])>0.:
               P8gen.SetParameters("411:addChannel      1  "+str(getbr_rpvsusy(h,'d_e',mass,couplings[0])/maxsumBR)+\
                                   "    0      -11       9900015")
               sumBR+=float(getbr_rpvsusy(h,'d_e',mass,couplings[0])/maxsumBR)
            if sumBR<1. and sumBR>0.:
            ...
            if getbr_rpvsusy(h,'ds_mu',mass,couplings[0])>0.:
               P8gen.SetParameters("431:addChannel      1  "+str(getbr_rpvsusy(h,'ds_mu',mass,couplings[0])/maxsumBR)+\
                                   "    0      -13       9900015")
               sumBR+=float(getbr_rpvsusy(h,'ds_mu',mass,couplings[0])/maxsumBR)
            if sumBR<1. and sumBR>0.:
                P8gen.SetParameters("431:addChannel      1   "+str(1.-sumBR)+"    0       22      -11")
            ...
            P8gen.SetParameters("411:new  D+ D-    1   3   0    1.86962"\
                                "    0.00000    0.00000    0.00000  3.11800e-01   0   1   0   1   0")
            sumBR=0.
            if getbr_rpvsusy(h,'d_mu',mass,couplings[0])>0.:
               P8gen.SetParameters("411:addChannel      1  "+str(getbr_rpvsusy(h,'d_mu',mass,couplings[0])/maxsumBR)+\
                                   "    0      -13       9900015")
               sumBR+=float(getbr_rpvsusy(h,'d_mu',mass,couplings[0])/maxsumBR)
            if sumBR<1. and sumBR>0.:
               P8gen.SetParameters("411:addChannel      1   "+str(1.-sumBR)+"    0       22      -11")
    ...
    if inclusive=="b":
        ...
        P8gen.SetParameters("9900015:isResonance = false")
        # Configuring decay modes...
        rpvsusy_instance.AddChannelsToPythia(P8gen) #add decay modes of RPV neutralino
        # Finish HNL setup...
        P8gen.SetParameters("9900015:mayDecay = on")
        P8gen.SetHNLId(9900015)
        # also add to PDG
        ...
        if bzerobplus == 1:
            if leptongeneration == 1:
                beautyhistograms = ['b_e','b_tau','b0_nu_e','b0_nu_tau']
            elif leptongeneration == 2 or leptongeneration == 3:
                beautyhistograms = ['b_tau']
        else:
            if leptongeneration == 1:
                beautyhistograms = ['b_e','b_tau','b0_nu_e','b0_nu_tau']
            elif leptongeneration == 2 or leptongeneration == 3:
                beautyhistograms = ['b0_nu_tau']
        ...
        print(maxsumBR, totalBR)

        if benchmark == 5 and bzerobplus == 1:
            #overwrite B+ decays #only in bench 5
            P8gen.SetParameters("521:new  B+               B-    1   3   0    5.27925"\
                                "0.00000    0.00000    0.00000  4.91100e-01   0   1   0   1   0")
            sumBR=0.

            if getbr_rpvsusy(h,'b_tau',mass,couplings[0])>0.:
               P8gen.SetParameters("521:addChannel      1  "+\
                                   str(getbr_rpvsusy(h,'b_tau',mass,couplings[0])/maxsumBR)+"    0       9900015      -15")
               sumBR+=float(getbr_rpvsusy(h,'b_tau',mass,couplings[0])/maxsumBR)
            print(sumBR,'sumbr')
            if sumBR<1. and sumBR>0.:
               P8gen.SetParameters("521:addChannel      1   "+\
                                   str(1.-sumBR)+"    0       22      22")
            print(getbr_rpvsusy(h,'b_tau',mass,couplings[0])/maxsumBR)
            
            P8gen.List(9900015)

        if benchmark == 5 and bzerobplus == 0:
            #overwrite B0 decays Alex
            ...
            if leptongeneration == 3:
                ...
                sumBR=0.
                if getbr_rpvsusy(h,'b0_nu_tau',mass,couplings[0])>0.:

                   P8gen.SetParameters("511:addChannel      1  "+\
                                       str(getbr_rpvsusy(h,'b0_nu_tau',mass,couplings[0])/maxsumBR)+\
                                       "   0       -16      9900015")
                   sumBR+=float(getbr_rpvsusy(h,'b0_nu_tau',mass,couplings[0])/maxsumBR)
                print(sumBR,'sumbr')

                if sumBR<1. and sumBR>0.:
                   P8gen.SetParameters("511:addChannel      1   "+\
                                       str(1.-sumBR)+"    0       22      22")
                print(getbr_rpvsusy(h,'b0_nu_tau',mass,couplings[0])/maxsumBR)
        ...
        if benchmark != 5:
            #overwrite B0 decays Alex
            if leptongeneration == 1:
                # 5.27958 is the mass GeV
                # 4.58700e-01 tau0 in mm/c properlifetime
                P8gen.SetParameters("511:new  B0  Bbar0    1   0   0    5.27958"\
                                    "    0.00000    0.00000    0.00000  4.58700e-01   0   1   0   1   0")
                sumBR=0.

                if getbr_rpvsusy(h,'b0_nu_e',mass,couplings[0])>0.:
                   P8gen.SetParameters("511:addChannel      1  "+\
                                       str(getbr_rpvsusy(h,'b0_nu_e',mass,couplings[0])/maxsumBR)+\
                                       "   0       -12      9900015") # ALEX
                   sumBR+=float(getbr_rpvsusy(h,'b0_nu_e',mass,couplings[0])/maxsumBR)
                ...
                if getbr_rpvsusy(h,'b0_nu_mu',mass,couplings[0])>0.:

                   P8gen.SetParameters("511:addChannel      1  "+\
                                       str(getbr_rpvsusy(h,'b0_nu_mu',mass,couplings[0])/maxsumBR)+\
                                       "   22       9900015      -14")
                   sumBR+=float(getbr_rpvsusy(h,'b0_nu_mu',mass,couplings[0])/maxsumBR)
