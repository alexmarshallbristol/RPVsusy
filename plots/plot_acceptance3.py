...
# print(np.load('/eos/experiment/ship/user/amarshal/RPV_output/NEW/grid_data_rpv_5_1_1_5000_8_0.npy'))
# quit()
files = glob.glob('/eos/experiment/ship/user/amarshal/RPV_output/NEW/grid_data_rpv_5_1_1_5000_*_0.npy')
...
for file in files:
	...
	# print(current)
	print(current[0][8], current[0][9], current[0][10], current[0][11])
quit()

# [2.66666667e+00 3.16227766e-04 0.00000000e+00 0.00000000e+00
#  3.16227766e-04 9.57306475e-16 8.02279918e-05 1.37097561e-02
#  9.57306475e-16 1.00000000e+02 0.00000000e+00 0.00000000e+00
#  1.40214465e-15 0.00000000e+00 3.46666194e-04]
...
events_in_5_years_all_b0 = np.zeros(np.shape(data_total)[0])
events_in_5_years_all_bplus = np.zeros(np.shape(data_total)[0])
events_in_5_years = np.zeros(np.shape(data_total)[0])


for i in range(0, np.shape(data_total)[0]):
		events_in_5_years_all_b0[i] = number_of_b0_mesons_SHiP*data_total[i][8]*data_total[i][9]/(50)
		events_in_5_years_all_bplus[i] = number_of_bplus_mesons_SHiP*data_total[i][10]*data_total[i][11]/50
		events_in_5_years[i] = events_in_5_years_all_bplus[i] + events_in_5_years_all_b0[i]




acceptance = np.divide(events_in_5_years,data_total[:,14])
masses = data_total[:,0]
...
plt.savefig('acceptance3',bbox_inches='tight')
...
# plt.ylabel(r'\textbf{$\frac{\lambd/a^\prime_{122}}{m^2_{\bar{f}}}=\frac{\lambda^\prime_{112}}{m^2_{\bar{f}}}$ in GeV$^{-2}$}', fontsize=18)
# plt.xlabel(r'\textbf{$m_{\widetilde{\chi}^0_1}$ in MeV}', fontsize=18, labelpad=15)
plt.ylabel('Coupling', fontsize=12)
plt.xlabel('Mass', fontsize=12, labelpad=12)
# plt.tight_layout()
plt.subplots_adjust(left=0.15, right=0.95, top=0.95, bottom=0.15)
plt.savefig('sensitivity3.png')
